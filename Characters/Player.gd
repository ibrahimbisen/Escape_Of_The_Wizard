...
func Laser():
	...
	emit_signal("player_fired_laser", gunpoint.global_transform.origin, direction)
	laser_instance.set_is_casting(true)
	laser_instance.get_node("Line2D").add_point(gunpoint.global_transform.origin)
	var fire_to = gunpoint.global_transform.origin + (direction*50)
	laser_instance.get_node("Line2D").add_point(fire_to)
	print(laser_instance.get_node("Line2D").points)
	laser_instance.cast_point = direction
>>>>>>> Stashed changes
